...
@section('content')
<form action="{{route('countyStore')}}"  method="post"class="form-2" align="center" enctype="multipart/form-data" >
<input type = "hidden" name = "_token" value = "<?php echo csrf_token(); ?>">
  ...
  <p class="float">
  <label for="countyNumber"><i class="icon-county"></i>County Number</label><br>
    <input id="countyNumber" type="text" placeholder="County Number " class="form-control @error('countyNumber') is-invalid @enderror" name="countyNumber" required autocomplete="fullname" autofocus>
    @error('fullname')
...
<label for="name"><i class="icon-county"></i>County Name</label><br>
  <input id="countyName" type="text" placeholder="County Name" class="form-control @error('CountyName') is-invalid @enderror"name="countyName" required autocomplete="countyName">
  @error('email')
      <span class="invalid-feedback" role="alert">
          <strong>{{ $message }}</strong>
      </span>
